...
int main() {
  ...
  h.onMessage([&cur_lane, &ref_vel,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // Get current s
						...
						//Handle the sensor fusion
						for (int i=0; i<sensor_fusion.size();i++){
							//Check if car in the lane
								 ...
								 if (d < (2+4*l+2) && d > (2 + 4*l -2) ){
							      ...
							      check_car_s += ((double)prev_size*(0.02)*check_speed);
										...
										if ((check_car_s>car_s)&&((check_car_s-car_s)<30.0)){
											 // Car in front of the car and too close
									     lane_notsafe[l] = true;
								    }
										else if (abs(check_car_s-car_s)<5.0){
											 // Car too close 
                       lane_notsafe[l] = true;
										}
										else{
											// Record the loss
                       double car_neardis = -abs(check_car_s-car_s);
											 if (car_neardis < lane_cost[l]){
												 lane_cost[l] = car_neardis;
											 }
            ...
            // Decide the lane and speed_ref
						if (lane_notsafe[cur_lane]){
							if (cur_lane == 0){
								if (lane_notsafe[1]){
                   ref_vel -= .224;
								}
                   ...
                   ref_vel -= .224;
								...
								if (lane_notsafe[0] && lane_notsafe[2]){
									ref_vel -= .224;
								}
								else if (!lane_notsafe[0] && !lane_notsafe[2]){
								 	cur_lane = (lane_cost[0]<=lane_cost[2])?0:2;
								}
								else if (!lane_notsafe[0]){
                  cur_lane = 0;
								}
								else{
                  cur_lane = 2;
            ...
            //Fit spline
						...
						/*
						for (int i =0; i < ptsx.size(); i++){
							cout << "ptxs " << ptsx[i] << " ptsy " << ptsy[i] << endl;
						}
						*/
